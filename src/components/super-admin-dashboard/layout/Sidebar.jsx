...
import { menuTree } from "@/temp_data/admin-menu";
import {
  ChevronDown,
  X,
} from "lucide-react";
...
export default function Sidebar({ open, setOpen, onLinkClick }) {
  ...
  const handleLinkClick = (item) => {
    // Call onLinkClick for all items that have links
        ...
        {item.link ? (
          ...
          <>
            ...
            {item.children && openMenus[item.id] && (
              <ul className={`ml-${level * 2 + 2} mt-1 space-y-1 transition-all duration-300`}>
