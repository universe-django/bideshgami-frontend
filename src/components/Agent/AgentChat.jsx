...
export default function MessagingApp() {
  ...
  return (
    <div className="flex h-screen bg-gray-100 w-full">
      <div className="flex max-w-7xl mx-auto  w-full bg-gray-100">
        {/* Sidebar */}
        <div className="w-80 rounded-s-lg bg-gray-50 border-r border-gray-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-gray-50">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`flex mb-5 ${
                  message.sent ? "flex-row-reverse" : ""
                }`}
              >
                <div
                  className={`w-9 h-9 rounded-full flex-shrink-0 ${
                    message.sent ? "bg-blue-500 ml-3" : "bg-gray-400 mr-3"
                  }`}
                />
                <div
                  className={`max-w-xl px-4 py-3 rounded-xl text-sm leading-relaxed shadow-sm ${
                    message.sent
